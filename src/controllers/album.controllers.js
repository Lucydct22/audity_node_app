...
async function putAlbumImage(req, res) {
  ...
  try {
    ...
    const imageUploaded = await uploadImage(
      ...
      `${cloudinaryConfig.folder}/albumImage`,
      250,
      250
    );
    if (!imageUploaded) {
...
async function updateAlbum(req, res) {
  ...
  try {
    ...
    const albumToUpdate = await db.Album.findByIdAndUpdate(
      { _id: albumId },
      {
        name,
        genres,
    ...
    await tracks?.forEach(async (track) => {
      await db.Track.findOneAndUpdate({ _id: track }, { album: albumId }, { returnOriginal: false })
        .lean()
        .exec();
    });
