  ...
  try {
    if (req.files?.image.tempFilePath) {
      const imageUploaded = await uploadImage(
        ...
        `${cloudinaryConfig.folder}/playlistImages`,
        250,
        250
      );
...
async function updatePlaylist(req, res) {
  ...
  try {
    const playlistToUpdate = await db.Playlist.findByIdAndUpdate({ _id: id }, { name, description })
      .lean()
      .exec();
    if (!playlistToUpdate) {
    ...
    return res
      .status(200)
      .send({ status: 200, playlist: playlistToUpdate, message: 'The playlist was updated' });
  } catch (err) {
    return res.status(500).send({ status: 500, error: err });
  }
}
...
async function updatePlaylistImage(req, res) {
  ...
  try {
    ...
    const imageUploaded = await uploadImage(
      req.files.image.tempFilePath,
      `${cloudinaryConfig.folder}/playlistImage`,
      250,
      250
    );
    if (!imageUploaded) {
