...
async function updateUserCountry(req, res) {
  ...
  try {
    const userToUpdated = await db.User.findOneAndUpdate({ userId: user.payload.sub.toString() }, { country })
      .lean()
      .exec();
...
async function getUserRole(req, res) {
  try {
    const userStored = await db.User.findOne({ userId: req.auth.payload.sub }).lean().exec();
...
async function deleteUser(req, res) {
  ...
  try {
    ...
    const userToDelete = await db.User.findOneAndDelete({ _id: userId.toString() }).lean().exec();
...
async function updateUserRole(req, res) {
  ...
  const { role } = req.body;
  try {
    const userToUpdated = await db.User.findOneAndUpdate({ _id: userId }, { role: role }).lean().exec();
    if (!userToUpdated) {
      return res.status(400).send({ status: 400 });
    }
    return res.status(200).send({ status: 200 });
