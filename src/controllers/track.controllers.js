...
async function postPrivateTrack(req, res) {
  ...
  try {
    ...
    if (artists) {
      track.uploadByUser.artists = artists;
    }
    const trackSaved = await track.save();
    if (!trackSaved) {
      return res.status(400).send({ status: 400 });
...
async function getTrackById(req, res) {
  ...
  try {
    const tracksStored = await db.Track.findOne({ _id: trackId })
      .populate('genres')
      .populate('artists')
      .populate('album')
      .populate('likedBy')
...
async function putTrackImage(req, res) {
  ...
  try {
    ...
    const imageUploaded = await uploadImage(
      ...
      `${cloudinaryConfig.folder}/trackImage`,
      250,
      250
    );
    if (!imageUploaded) {
...
async function putTrackAudio(req, res) {
  ...
  try {
    ...
    if (findTrack.audioPublicId) await removeMedia(findTrack.audioPublicId, 'video');
    const audioUploaded = await uploadAudio(
      req.files.audio.tempFilePath,
      `${cloudinaryConfig.folder}/trackAudio`
    );
    if (!audioUploaded) {
      return res.status(402).send({ status: 402 });
    }
