...
async function getBaseAlbum() {
  ...
  const albums = [
    {
      ...
      genres: [genre[0].toString(), genre[1].toString()],
    ...
    },
    {
      ...
      artists: [artist[1].toString()],
      genres: [genre[3].toString(), genre[4].toString()],
      likedBy: [user[0].toString(), user[1].toString()],
    },
    {
      _id: new ObjectId(),
      ...
      artists: [artist[2].toString()],
      genres: [genre[5].toString(), genre[6].toString()],
      likedBy: [user[0].toString(), user[1].toString()],
    },
    {
      _id: new ObjectId(),
      ...
      genres: [genre[7].toString(), genre[8].toString()],
      likedBy: [user[0].toString(), user[1].toString()],
    },
    {
      _id: new ObjectId(),
      ...
      genres: [genre[9].toString(), genre[10].toString()],
      likedBy: [user[0].toString(), user[1].toString()],
    },
    {
      _id: new ObjectId(),
      ...
      genres: [genre[0].toString(), genre[0].toString()],
      likedBy: [user[0].toString(), user[1].toString()],
    },
    {
      _id: new ObjectId(),
  ...
  albums.forEach((album) => {
    album.genres.forEach(async (genreId) => {
      await db.Genre.findOneAndUpdate({ _id: genreId }, { $push: { albums: album._id } })
        .lean()
        .exec();
    });

    album.artists.forEach(async (artistId) => {
      await db.Artist.findOneAndUpdate({ _id: artistId }, { $push: { albums: album._id } })
        .lean()
        .exec();
    });

    album.likedBy.forEach(async (likeId) => {
      await db.User.findOneAndUpdate({ _id: likeId }, { $push: { 'likesTo.albums': album._id } })
        .lean()
        .exec();
    });
  });
