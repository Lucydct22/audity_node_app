...
async function getBasePlaylist() {
  ...
  const playlists = [
    {
      ...
      followers: 100,
      rating: 5,
      likedBy: [user[0].toString()],
      followedBy: [user[0].toString(), user[1].toString()],
      artists: [artist[0].toString(), artist[5].toString()],
      genres: [genre[0].toString(), genre[2].toString()],
    },
    {
      _id: new ObjectId(),
      userId: user[0].toString(),
      ...
      followedBy: [user[0].toString(), user[1].toString()],
      artists: [artist[0].toString(), artist[5].toString()],
      genres: [genre[0].toString(), genre[5].toString()],
    },
    {
      _id: new ObjectId(),
      userId: user[0].toString(),
    ...
    },
    {
      ...
      followedBy: [user[0].toString(), user[1].toString()],
      artists: [artist[0].toString(), artist[5].toString()],
      genres: [genre[3].toString(), genre[7].toString()],
    },
    {
      _id: new ObjectId(),
      userId: user[0].toString(),
    ...
    },
    {
      ...
      followedBy: [user[0].toString(), user[1].toString()],
      artists: [artist[0].toString(), artist[5].toString()],
      genres: [genre[3].toString(), genre[6].toString()],
    },
    {
      _id: new ObjectId(),
      userId: user[0].toString(),
  ...
  playlists.forEach((playlist) => {
    playlist.genres.forEach(async (genreId) => {
      await db.Genre.findOneAndUpdate({ _id: genreId }, { $push: { playlists: playlist._id } })
        .lean()
        .exec();
    });

    playlist.artists.forEach(async (artistId) => {
      await db.Artist.findOneAndUpdate({ _id: artistId }, { $push: { playlists: playlist._id } })
        .lean()
        .exec();
    });

    playlist.likedBy.forEach(async (likeId) => {
      await db.User.findOneAndUpdate({ _id: likeId }, { $push: { 'likesTo.playlists': playlist._id } })
        .lean()
        .exec();
    });
  });
